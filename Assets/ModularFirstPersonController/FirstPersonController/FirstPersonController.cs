...
//
// "Enable/Disable Headbob, Changed look rotations - should result in reduced camera jitters" || version 1.0
...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float actualStepTime = 0f;
         ...
         if (playerCanMove)
    {
        ...
        targetVelocity = transform.TransformDirection(targetVelocity) * actualSpeed;
        ...
        else
        {
            ...
            rb.linearVelocity = Vector3.Lerp(rb.linearVelocity, direction * 0f, Time.deltaTime * 3f); 
        }

        // Vérifie si le joueur marche et est au sol pour activer le Head Bob
        ...
        // Gère les mouvements pendant le sprint
        if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
        {
            actualSpeed += acceleration;
            actualSpeed = Mathf.Min(actualSpeed, sprintSpeed);

            // Applique une force pour atteindre la vitesse cible
        ...
        {
            isSprinting = false;
            // Calcul des mouvements pendant la marche
            actualSpeed = Mathf.Lerp(walkSpeed, actualSpeed, Time.deltaTime * 20f); // Lerp pour une transition douce vers la vitesse de marche
            actualSpeed = Mathf.Clamp(actualSpeed, walkSpeed, sprintSpeed);
    ...
    private void HeadBob()
    {
        if(isWalking)
        {
            ...
            if(isSprinting)
            {
                timer += Time.deltaTime * (bobSpeed + actualSpeed);
            }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.sprintSpeed = EditorGUILayout.Slider(new GUIContent("Sprint Speed", "Determines how fast the player will move while sprinting."), fpc.sprintSpeed, fpc.walkSpeed, 60f);
