...
namespace RayFireEditor
{
    ...
    public class RayfireRigidEditor : Editor
    {
        ...
        void SetupUI()
        {
            ...
            if (GUILayout.Button (TextRig.gui_btn_edt_setup, GUICommon.buttonStyle, GUILayout.Height (25)))
                foreach (var targ in targets)
                    if (targ as RayfireRigid != null)
                    {
            ...
            if (GUILayout.Button (TextRig.gui_btn_edt_reset, GUICommon.buttonStyle, GUILayout.Height (25)))
                foreach (var targ in targets)
                    if (targ as RayfireRigid != null)
                    {
        ...
        void GUI_Cluster_Preview()
        {
            if (rigid.objTp == ObjectType.ConnectedCluster)
            {
                EditorGUI.BeginChangeCheck();
                GUILayout.BeginHorizontal();
                GUICommon.Toggle (sp_cls_cn, TextRig.gui_btn_conns, GUICommon.color_btn_blue);
                GUICommon.Toggle (sp_cls_nd, TextRig.gui_btn_nodes, GUICommon.color_btn_blue);
                EditorGUILayout.EndHorizontal();
